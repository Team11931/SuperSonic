 ...
 * THIS IS THE NAVIGATION CLASS FOR TEAM SUPERSONIC'S ROBOT
...
public class NavigationSupersonic {
    ...
    private static final VuforiaLocalizer.CameraDirection CAMERA_CHOICE = BACK; // Select which camera you want use; Valid choices are BACK or FRONT. (The FRONT camera is the one on the same side as the screen).
    ...
    public NavigationSupersonic(int cameraMonitorViewId) {
        ...
        // VuforiaLocalizer.Parameters parameters = new VuforiaLocalizer.Parameters(); // WithOUT camera preview
    ...
    public boolean trackTargets(){
        ...
        if (targetVisible) {
            ...
            //opMode.telemetry.addData("Pos (in)", "{X, Y, Z} = %.1f, %.1f, %.1f", translation.get(0) / mmPerInch, translation.get(1) / mmPerInch, translation.get(2) / mmPerInch);
